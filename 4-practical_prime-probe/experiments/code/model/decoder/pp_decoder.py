...
class PP_Trace2IDCT_01_DecNET(nn.Module):
    def __init__(self, dim, nc, padding_type='reflect', norm_layer=nn.BatchNorm2d, use_dropout=False, use_bias=False):
        super().__init__()
        ...
        self.convTFinal = nn.Sequential(
            nn.ConvTranspose2d(self.dim, self.outc, 4, 2, 1, bias=False),
            nn.Sigmoid()
        )
        self.apply(weights_init)
    ...
    def forward(self, x):
        ...
        x = self.convT1(x)
        x = self.convTFinal(x)
        x = x.view(x.shape[0], x.shape[1], 64)
