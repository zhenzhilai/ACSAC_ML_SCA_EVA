...
class Trainer:
    ...
    def train(self, epoch, data_loader, model):
        ...
        for batch_idx in tbar:
            ...
            (0.84 * train_loss + 0.16* train_loss_mse).backward()
    ...
    def train_idct_new(self, epoch, data_loader, model):
        ...
        for batch_idx in tbar:
            ...
            for i in range(len(model.optimiser.param_groups)):
                model.optimiser.param_groups[i]['lr'] = curr_lr
            tbar.set_description('idx {}/{} loss {} metrics {} curr_lr {}'.format(batch_idx, self.param.iters_per_epoch,
                                                                       train_loss.item(), 0, model.optimiser.param_groups[0]['lr']))
