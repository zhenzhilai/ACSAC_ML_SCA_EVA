...
def main(hyp_var):
    ...
    if hyp_var.target == 'idct':
        if not hyp_var.recon_trace_to_img:
            if hyp_var.idct_out_dim == 1:
                train_loader = SideChannel(pp_dataset.PP_SideChannelDataset_RealTrace_to_IDCT_oneline(hyp_var.data_dir, mode='train', ID_file=hyp_var.ID_file, sidechannel_subdir= hyp_var.subdir, dim=hyp_var.dim), config=hyp_var)
                test_loader = SideChannel(pp_dataset.PP_SideChannelDataset_RealTrace_to_IDCT_oneline(hyp_var.data_dir, mode='test', ID_file=hyp_var.ID_file, sidechannel_subdir= hyp_var.subdir, dim=hyp_var.dim),
    ...
    if hyp_var.recon_trace_to_img:
        ...
        for ep in range(hyp_var.epochs):
            test_loader = SideChannel(pp_dataset.PP_SideChannelDataset_Recons_IDCT(hyp_var.data_dir, mode='test', ID_file=hyp_var.ID_file, sidechannel_subdir= hyp_var.subdir, dim=hyp_var.dim, epoch=ep),
                                # update drop_last and shuffle
                                config=EasyDict({**hyp_var, **{'drop_last': False, 'shuffle': False}}))
            network = load_checkpoint(network, trace_to_idct_to_img_model_dir, ep)
            network.eval()
            validator.valid(ep, data_loader=test_loader, model=network, img_out_path=hyp_var.img_out_path)
        return False
