...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self, attention_dim, embed_dim, decoder_dim, vocab_size, encoder_dim=2048, dropout=0.5, device=None):
        """
...
class SC09_embed_128(nn.Module): # for SC09
    def __init__(self, dim=128, nc=1):
        super(SC09_embed_128, self).__init__()
        self.dim = dim
        nf = 64
...
class Sub_URMP_embed_128(nn.Module): # for Sub-URMP
    def __init__(self, dim=128, nc=1):
        super(Sub_URMP_embed_128, self).__init__()
        self.dim = dim
        nf = 64
...
class attn_30w_trace_encoder_256(nn.Module):
    def __init__(self, dim, nc=1):
        super(attn_30w_trace_encoder_256, self).__init__()
        self.dim = dim
        nf = 64
        ...
        # input is (nc) x 256 x 256
        self.c1 = nn.Sequential(
            nn.Conv2d(nc, nf, (8, 4), (4, 2), 1),
            nn.BatchNorm2d(nf),
            nn.LeakyReLU(0.2, inplace=True),
            )
        # state size. (nf) x 128 x 128
        self.c2 = nn.Sequential(
            nn.Conv2d(nf, nf, (8, 4), (4, 2), 1),
            nn.BatchNorm2d(nf),
            nn.LeakyReLU(0.2, inplace=True),
            )
        # state size. (nf) x 64 x 64
        self.c3 = nn.Sequential(
            nn.Conv2d(nf, nf*2, (8, 3), (4, 1), 1),
            nn.BatchNorm2d(nf*2),
            nn.LeakyReLU(0.2, inplace=True),
            )
        # state size. (nf*2) x 32 x 32
        self.c4 = nn.Sequential(
            nn.Conv2d(nf*2, nf*4, (8, 3), (4, 1), 1),
            nn.BatchNorm2d(nf*4),
            nn.LeakyReLU(0.2, inplace=True),
            )
        # state size. (nf*4) x 16 x 16
        self.c5 = nn.Sequential(
            nn.Conv2d(nf*4, nf*8, (8, 4), (4, 2), 1),
            nn.BatchNorm2d(nf*8),
            nn.LeakyReLU(0.2, inplace=True),
            )
        # state size. (nf*8) x 8 x 8
        self.c6 = nn.Sequential(
            nn.Conv2d(nf*8, nf*8, (8, 4), (4, 2), 1),
            nn.BatchNorm2d(nf*8),
            nn.LeakyReLU(0.2, inplace=True),
            )
        # # state size. (nf*8) x 4 x 4
