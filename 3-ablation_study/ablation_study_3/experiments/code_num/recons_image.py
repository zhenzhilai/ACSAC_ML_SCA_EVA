...
wandb.init(
    ...
    name="Manifold_GAN L1D New_Loss A6000",
    
    mode="disabled",

    save_code=True,

    notes="Replicate Manifold-SCA with the same dataset: AVX idct, 6 x 256 x 256; AE only"
)
    ...
    def train(self, data_loader):
        with torch.autograd.set_detect_anomaly(True):
            ...
            for i in tbar:
                ...
                recons_ssim_value = pytorch_ssim.ssim(decoded, image).detach()

                #(errG + errC_fake + self.args.lambd * recons_err).backward()
                (errG + errC_fake + self.args.lambd * (0.16 * recons_err + 0.84 * recons_ssim_value) ).backward()
                #(0.15 * recons_err + 0.85 * recons_ssim_value).backward()
                #(recons_err).backward()
            ...
            # utils.clear_progressbar()
                ...
                #tbar.set_description('Epoch %d, Loss %.4f' % (self.epoch, record.mean()))
                #wandb.log({"epoch": self.epoch, "loss": record.mean()})
